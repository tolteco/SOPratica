	...
	if (argc == 5){ //Verifica se a quantidade de argumentos e valida
		...
		if (d != NULL){
		  ...
		  printf ( "Current local time and date: %s\n", asctime (timeinfo) );
			...
			root.criacao = (root.criacao + (timeinfo->tm_sec / 2)) << 6;
			root.criacao = (root.criacao + (timeinfo->tm_min)) << 5;
			root.criacao = root.criacao + timeinfo->tm_hour;
			...
			size_t wb = fwrite(&root, sizeof(Centry_t), 1, d);
			fwrite(&root2, sizeof(Centry_t), 1, d);
      ...
      printf("  [novo label] Nome dado a unidade apos a formatacao (maximo de 12 caracteres)\n");
